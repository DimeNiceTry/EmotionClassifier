...
def process_message(ch, method, properties, body, worker_id, db):
    ...
    try:
        ...
        logger.info(f"Получено сообщение для анализа эмоций. ID: {data.get('prediction_id', 'unknown')}")
        ...
        # Выполняем предсказание эмоций на изображении
        prediction_result = make_prediction(input_data)
        
        # Добавляем информацию о времени обработки
        processing_time = (datetime.now() - processing_start).total_seconds()
        logger.info(f"Анализ эмоций для {prediction_id} выполнен за {processing_time:.2f} сек.")
